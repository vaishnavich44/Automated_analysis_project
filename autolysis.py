...
def perform_clustering(data, numeric_cols):
    ...
    Args:
        data (pd.DataFrame): The dataset to analyze.
        numeric_cols (list): List of numeric column names for clustering.
    """
    if not numeric_cols:
    ...
    try:
        data_numeric = data[numeric_cols].dropna().reset_index(drop=True)
        scaler = StandardScaler()
        data_numeric_scaled = scaler.fit_transform(data_numeric)
        
        kmeans = KMeans(n_clusters=3, random_state=42)
        clusters = kmeans.fit_predict(data_numeric_scaled)
...
def apply_pca(data, numeric_cols):
    ...
    Args:
        data (pd.DataFrame): The dataset to analyze.
        numeric_cols (list): List of numeric column names for PCA.
    """
    if not numeric_cols:
...
def generate_visualizations(data, numeric_cols, categorical_cols):
    ...
    if categorical_cols:
        ...
        sns.barplot(x=top_categories.values, y=top_categories.index, palette="viridis", hue=top_categories.index, dodge=False)
...
def generate_dynamic_prompt(data, numeric_cols, categorical_cols, outliers, visualizations):
    """
    Generates a dynamic prompt for README.md content creation.
    Args:
        data (pd.DataFrame): The dataset analyzed.
        numeric_cols (list): Numeric columns in the dataset.
        categorical_cols (list): Categorical columns in the dataset.
        outliers (dict): Outlier summary.
        visualizations (list): Generated visualization file paths.
    Returns:
